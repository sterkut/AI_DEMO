...
# Змінюємо назву в налаштуваннях сторінки
st.set_page_config(page_title="AI Sales Audit | Demo Case", layout="wide")

# Після df = load_data() додаємо цей блок:
...
def load_data():
    ...
    try:
        ...
        url = "https://docs.google.com/spreadsheets/d/1ngMU3jHZV_zFP6RSOeIjxMBs6xd0PTE7ijD09kgBeKw/edit?gid=0#gid=0"
...
st.markdown("---")
    ...
    **Зв'язатися з розробником:**
    [Написати в Telegram](https://t.me/ТВІЙ_НІК)
    """)
df_filtered = df[
...
st.markdown("---")
    st.warning("💡 **Висновок для інвестора:** Впровадження цієї системи дозволяє повернути від 15% до 40% 'злитих' угод вже в перший місяць за рахунок виправлення помилок менеджерів.")
